...
function Login() {
  ...
  // Si déjà connecté, rediriger vers la page Cours
  useEffect(() => {
    if (user && !isLoading) {
      navigate(PATHS.app.courses.list, { replace: true });
    }
    ...
    <>
      <form method='get' action='#' className='flex flex-col gap-1 pt-4' onSubmit={handleSubmit}>
        ...
        <button
          type="submit"
          disabled={isLoading}
          className="mt-3 flex-center-center rounded-4xl h-11 text-white bg-brand font-bold text-lg disabled:opacity-70"
        >
          {isLoading ? "Connexion..." : t("login.signIn")}
        </button>
